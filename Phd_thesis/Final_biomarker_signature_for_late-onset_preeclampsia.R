...
ano <- pData(detrendedSTORKdataset)          # phenodata
prot <- fData(detrendedSTORKdataset)         # featuredata 
colnames(dta) <- prot$AptName
dta <- dta[which(ano$PEgroups %in% c(0,2)),]  # select only late-onset preeclampsia samples' protein abundances
ano <- ano[which(ano$PEgroups %in% c(0,2)),]  # select only late-onset preeclampsia samples in phenodata 


stabc060_alpha04list <- list()
BMS_list <- list()
for (tp in 1:3){                             # for each time point in tp, run stability selection algorithm and fit 
                                             # logistic regression model and save coefficients which make out the biomarker signature
  alph <- alphas[tp]       
  cl.columns <- c("BMI","Age","Nulliparity") # which clinical variables to include
  sub.idx <- which(ano$TimePoint==tp)        # select timepoint tp
  clinical <- ano[sub.idx,cl.columns]
  xmx <- cbind(dta[sub.idx,],clinical) %>% data.matrix()
  set.seed(1234,kind="default")
  spath <- stabpath(y=ano$PE[sub.idx],       # run stability selection algorithm 
                    x=xmx, 
                    size = 0.632, 
                    steps = 100,
                    mc.cores=1, 
  ...
  stabrun <- c060::stabsel(x=spath, error=1, type="pfer", pi_thr=0.6)  # use pi_thr = 0.6 to select the features that have a probability of at least 0.6 to be included in the model
